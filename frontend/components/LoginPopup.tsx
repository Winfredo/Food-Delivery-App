...
const LoginPopup = ({ setShowLoginPopup }: LoginPopupProps) => {
  ...
  const onLogin = async (e: any) => {
    e.preventDefault();
    let newUrl =
      url + (currentState === "Login" ? "/api/user/login" : "/api/user/signup");
  ...
  return (
    <div className="fixed inset-0 bg-[#00000090] bg-opacity-50 flex items-center justify-center z-50">
      ...
      >
        ...
        <button
          type="submit"
          className="w-full mt-4 cursor-pointer bg-[#FF6347] hover:bg-[#e5533d] text-white py-2 rounded transition-colors text-sm font-medium"
        >
        ...
        {currentState === "Sign Up" && (
          <div className="flex items-start gap-2 mt-3">
            <input
              type="checkbox"
              required
              className="mt-1 w-3.5 h-3.5 accent-[#FF6347]"
            />
