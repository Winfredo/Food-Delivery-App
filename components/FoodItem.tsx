  ...
  return (
    <div className='w-full m-auto rounded-[15px] shadow-[0px_0px_10px_#00000015] animation-fadeIn cursor-pointer'>
      <div className='relative'>
        ...
        {!cartItems?.[_id] ? <img className='absolute right-2 cursor-pointer bottom-2 w-8' onClick={()=>addToCart(_id)} src="/assets/add_icon_white.png" alt='add icon' /> 
            :
             <div className='flex items-center justify-start gap-1 absolute right-2 cursor-pointer bottom-2 bg-white rounded-full'>
                <img className='w-8 m-1' onClick={()=>removeFromCart(_id)} src="/assets/remove_icon_red.png" alt='remove icon' />
                <p>{cartItems[_id]}</p>
                <img className='w-8 m-1' onClick={()=>addToCart(_id)} src="/assets/add_icon_green.png" alt='add icon' />
