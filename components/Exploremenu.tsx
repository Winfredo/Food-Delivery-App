  ...
  return (
    <div className="flex flex-col gap-5">
      ...
      {menuList.map((menu, index) => (
        <div onClick={() => setCategory?.((prev: string) => prev === menu.name ? "All" : menu.name)} key={index} className="flex flex-col items-center mx-2.5">
          <Image
            src={menu.image}
          ...
          />
          <h3 className="mt-2 text-[#747474] font-semibold text-[1.2vw] cursor-pointer">{menu.name}</h3>
        </div>
      ))}
